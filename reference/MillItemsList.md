# Millénaire Mod 已注册物品列表

以下为旧 mod 中 `MillItems.java` 中注册的所有 item（部分为简要名称，具体属性可查源码）：

## 基础物品

- summoningwand
- negationwand
- denier
- denierargent
- denieror
- purse

## 种子类

- rice
- turmeric
- maize
- grapes
- cotton

## 工具类

- normanpickaxe
- normanaxe
- normanshovel
- normanhoe
- mayanpickaxe
- mayanaxe
- mayanshovel
- mayanhoe
- byzantinepickaxe
- byzantineaxe
- byzantineshovel
- byzantinehoe

## 武器类

- normanbroadsword
- mayanmace
- tachisword
- byzantinemace
- inuittrident
- seljukscimitar

## 弓类

- yumibow
- inuitbow
- seljukbow

## 护甲类

- normanhelmet
- normanplate
- normanlegs
- normanboots
- japanesebluehelmet
- japaneseblueplate
- japanesebluelegs
- japaneseblueboots
- japaneseredhelmet
- japaneseredplate
- japaneseredlegs
- japaneseredboots
- japaneseguardhelmet
- japaneseguardplate
- japaneseguardlegs
- japaneseguardboots
- byzantinehelmet
- byzantineplate
- byzantinelegs
- byzantineboots
- furhelmet
- furplate
- furlegs
- furboots
- seljukhelmet
- seljukturban
- seljukplate
- seljuklegs
- seljukboots

## 食物类

- ciderapple
- olives
- oliveoil
- cider
- calva
- boudin
- tripes
- vegcurry
- chickencurry
- rasgulla
- masa
- wah
- balche
- sikilpah
- cacauhaa
- udon
- sake
- ikayaki
- feta
- souvlaki
- winebasic
- winefancy
- bearmeat_raw
- bearmeat_cooked
- wolfmeat_raw
- wolfmeat_cooked
- seafood_raw
- seafood_cooked
- inuitpotatostew
- inuitmeatystew
- inuitbearstew
- ayran
- yogurt
- pide
- lokum
- helva
- pistachios
- cherries
- cherry_blossom

## 壁饰类

- tapestry
- indianstatue
- mayanstatue
- byzantineiconsmall
- byzantineiconmedium
- byzantineiconlarge
- hidehanging
- wallcarpetsmall
- wallcarpetmedium
- wallcarpetlarge

## 颜料桶

- paint*bucket*<颜色>（每种 EnumDyeColor 均有一个）

## 羊皮纸类

- parchment_normanvillagers
- parchment_normanbuildings
- parchment_normanitems
- parchment_normanfull
- parchment_indianvillagers
- parchment_indianbuildings
- parchment_indianitems
- parchment_indianfull
- parchment_mayanvillagers
- parchment_mayanbuildings
- parchment_mayanitems
- parchment_mayanfull
- parchment_japanesevillagers
- parchment_japanesebuildings
- parchment_japaneseitems
- parchment_japanesefull
- parchment_villagescroll

## 其他

- brickmould
- obsidianflake
- silk
- clothes_byz_wool
- clothes_byz_silk
- clothes_seljuk_wool
- clothes_seljuk_cotton
- vishnu_amulet
- alchemist_amulet

> 注：如需详细属性、注册代码或补充 block 相关 item，请查阅 `MillItems.java` 及 `MillBlocks.java`。
